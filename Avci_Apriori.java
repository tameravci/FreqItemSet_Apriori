 ...
 * T10I4D100K.dat with minimum support 500: <3 seconds
 * chess.dat with minimum support 2500 <5 seconds
 */
import java.io.*;
import java.util.*;
				...
				for(int i=0; i< row.size()-1; i++) {
					for(int j=i+1; j<row.size(); j++) {
						...
						if(!twoItemSet.containsKey(key)) {
							twoItemSet.put(key, new Avci_Apriori().new dataPoint(1, itemset));
				...
				boolean match = true;
					...
					if(match) { //match k-2 elements
						...
						if(currentSet.containsKey(Arrays.toString(X)) && currentSet.containsKey(Arrays.toString(Y)))
							candidateSet.put(Arrays.toString(C), new Avci_Apriori(). new dataPoint(0, C));	
					}
